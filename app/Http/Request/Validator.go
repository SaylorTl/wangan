...
func InitTrans(locale string) (err error) {
	...
	if v, ok := binding.Validator.Engine().(*validator.Validate); ok {
		...
		v.RegisterTagNameFunc(func(fld reflect.StructField) string {
			name := strings.SplitN(fld.Tag.Get("zh_cn"), ",", 2)[0]
			if name == "-" {
				return ""
			}
			return name
		...
		_ = v.RegisterTranslation("ip_valid_rule", trans, func(ut ut.Translator) error {
			return ut.Add("ip_valid_rule", "{0}格式错误!", true)
		}, func(ut ut.Translator, fe validator.FieldError) string {
			t, _ := ut.T("ip_valid_rule", fe.Field())
			return t
		})

		_ = v.RegisterTranslation("keyword_val", trans, func(ut ut.Translator) error {
			return ut.Add("keyword_val", "{0}格式错误!", true)
		}, func(ut ut.Translator, fe validator.FieldError) string {
			t, _ := ut.T("keyword_val", fe.Field())
			return t
		})
...
func addValueToMap(fields map[string]string) string {
